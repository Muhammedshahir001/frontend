...
const ProductCard = ({ product }) => {
  ...
  return (
    <motion.div
      ...
      viewport={{ once: true }}
      className="group bg-white flex flex-col h-full rounded-[16px] md:rounded-[24px] overflow-hidden border border-slate-100 transition-all duration-500 ease-in-out hover:-translate-y-3 hover:shadow-[0_30px_60px_-15px_rgba(15,23,42,0.15)] relative shadow-[0_8px_20px_-10px_rgba(0,0,0,0.08)] w-full max-w-full sm:max-w-[480px] mx-auto"
      onMouseEnter={() => {
    ...
    >
      ...
      <div className="p-4 md:p-8 flex flex-col flex-grow items-center text-center">
        
        {/* Rating Stars - screenshot style */}
        <div className="flex items-center gap-1.5 md:gap-2 mb-2 md:mb-5">
          ...
          <span className="text-[11px] md:text-[16px] text-slate-400 font-bold">
            ({reviews})
          </span>
        </div>
        ...
        <Link to={productLink} className="block w-full">
          <h3 className="text-[14px] md:text-[24px] font-black text-[#0f172a] mb-2 md:mb-5 leading-tight min-h-[40px] md:min-h-[60px] hover:text-blue-700 transition-colors px-1 md:px-2 line-clamp-2 md:line-clamp-none">
            {product.name}
          </h3>
        </Link>
        ...
        <div className="mt-auto w-full pt-1 md:pt-2">
          <div className="flex items-baseline justify-center gap-2 md:gap-4 mb-2 md:mb-4">
            <span className="text-[18px] md:text-[38px] font-black text-[#0f172a] tracking-tight">
              {IndianRupee.format(currentPrice)}
            </span>
            {hasOffer && (
              <span className="text-[12px] md:text-[21px] font-bold text-slate-400 line-through">
          ...
          <div className="min-h-[30px] md:min-h-[60px] flex flex-col items-center justify-center gap-1 md:gap-2">
            <p className="text-[9px] md:text-[14px] font-black text-[#dc2626] uppercase tracking-[0.05em] md:tracking-[0.1em]">
              {product.offer || 'EXCLUSIVE DEAL'}
            </p>
            <p className="text-[10px] md:text-[14px] text-slate-500 font-semibold hidden md:block">
              {product.features || ''}
            </p>
          </div>
        </div>
      ...
      <div className="px-4 md:px-8 pb-4 md:pb-9 mt-auto">
        <button
          onClick={handleAddToCart}
          className="w-full bg-[#0f172a] text-white py-[12px] md:py-[20px] rounded-full text-[10px] md:text-[16px] font-black uppercase tracking-[0.1em] md:tracking-[0.2em] transition-all duration-300 ease-in-out hover:bg-blue-700 hover:scale-[1.02] active:scale-[0.98] hover:shadow-2xl hover:shadow-blue-700/30 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2"
        >
          Add to Cart
        </button>
      </div>
